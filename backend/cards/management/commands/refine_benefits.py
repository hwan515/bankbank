"""
카드 혜택 텍스트를 LLM으로 정규화하여 structured_benefit에 저장하는 Django command

Usage:
    python manage.py refine_benefits              # structured_benefit이 비어있는 카드만 처리
    python manage.py refine_benefits --all        # 모든 카드 재처리
    python manage.py refine_benefits --limit 10   # 최대 10개만 처리
    python manage.py refine_benefits --card-id 123  # 특정 gorilla_id만 처리
"""
import os
import re
import json
...
# backend/.env 또는 analyze_card/.env에서 환경변수 로드
# refine_benefits.py -> commands -> management -> cards -> backend -> final-pjt
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent.parent
load_dotenv(PROJECT_ROOT / 'backend' / '.env')  # backend/.env
load_dotenv(PROJECT_ROOT / 'analyze_card' / '.env')  # analyze_card/.env (fallback)
...
def build_input_text(benefits_json: list, max_chars: int) -> Optional[str]:
    if not isinstance(benefits_json, list):
        return None

    lines: List[str] = []
    for item in benefits_json:
        if not isinstance(item, dict):
            continue

        title = str(item.get("title", "")).strip()
        if any(k in title for k in [
            "유의사항", "꼭 확인", "꼭 확인하세요", "안내사항", "주의사항",
            "참고사항", "참고", "전월 이용금액 기준", "전년도 이용금액 기준",
            "전월 이용금액 제외", "전년도 이용금액 제외", "할인 제외 대상", "적립 제외 대상"
        ]):
            continue
...
def call_llm_structured_with_retry(client: OpenAI, text: str) -> Tuple[str, List[Dict[str, Any]]]:
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        # 1) Responses API + text.format(json_schema)
        try:
...
class Command(BaseCommand):
    help = '카드 혜택 텍스트를 LLM으로 정규화하여 structured_benefit에 저장'

    def add_arguments(self, parser):
        parser.add_argument(
            '--all',
            action='store_true',
            help='모든 카드 재처리 (기본: structured_benefit이 비어있는 카드만)'
        )
        parser.add_argument(
            '--limit',
            type=int,
            default=0,
            help='처리할 최대 카드 수 (0=무제한)'
        )
        parser.add_argument(
            '--card-id',
            type=int,
            default=0,
            help='특정 gorilla_id만 처리'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        ranking_min = options['ranking_min']
        ranking_max = options['ranking_max']
        ...
        # 카드 쿼리
        if card_id:
            cards = Card.objects.filter(gorilla_id=card_id)
        elif process_all:
            cards = Card.objects.all().order_by('ranking', 'id')
        else:
            # structured_benefit이 비어있는 카드만
            cards = Card.objects.filter(structured_benefit=[]).order_by('ranking', 'id')

        # ranking 필터 적용
        if ranking_min > 0:
            cards = cards.filter(ranking__gte=ranking_min)
        if ranking_max > 0:
            cards = cards.filter(ranking__lte=ranking_max)

        if limit > 0:
            cards = cards[:limit]

        cards = list(cards)
        self.stdout.write(f'🤖 LLM으로 카드 {len(cards)}개 분석 시작')
        ...
        for idx, card in enumerate(cards, start=1):
            text = build_input_text(card.benefits_json, MAX_INPUT_CHARS)

            if not text:
                self.stdout.write(f'[{idx}] {card.name}: 혜택 데이터 없음 (Skip)')
                fail += 1
                continue

            self.stdout.write(f'[{idx}] {card.name} 분석 중...', ending=' ')
            ...
            try:
                ...
                MAX_OUTPUT_ITEMS = 120

                for x in items[:MAX_OUTPUT_ITEMS]:
                    if isinstance(x, dict):
                        normalized.append(normalize_item(x, card.min_spending))

                if not normalized:
                    self.stdout.write(self.style.WARNING('결과 없음 (Empty) ⚠️'))
                    fail += 1
                    continue

                if not dry_run:
                    card.structured_benefit = normalized
                    card.save(update_fields=['structured_benefit', 'updated_at'])

                ok += 1
                self.stdout.write(self.style.SUCCESS(f'성공 ({len(normalized)}개 추출) ✅'))

                if not dry_run:
                    # 계산 필드 동기화
                    if card.sync_computed_fields():
                        card.save(update_fields=['annual_fee_min', 'categories', 'updated_at'])
